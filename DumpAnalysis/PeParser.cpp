...
bool PeParser::readPeHeaderFromProcess(bool readSectionHeaders)
{
	...
	if (CdbHelper::GetInstance()->GetData((PVOID)moduleBaseAddress, readSize, headerMemory))
	{
		retValue = true;
...
bool PeParser::readSectionFrom(const DWORD_PTR readOffset, PeFileSection & peFileSection, const bool isProcess)
{
	...
	while(currentOffset >= readOffset) //start from the end
	{
		...
		if (isProcess)
		{
			retValue = CdbHelper::GetInstance()->GetData((PVOID)currentOffset, currentReadSize, data);
		}
		else
		{
...
bool PeParser::readPeSectionFromProcess(DWORD_PTR readOffset, PeFileSection & peFileSection)
{
	...
	return CdbHelper::GetInstance()->GetData((PVOID)readOffset, peFileSection.dataSize, peFileSection.data);
}
...
typedef PIMAGE_NT_HEADERS(__stdcall* Fun_CheckSumMappedFile)(
	...
	if(hFileToMap != INVALID_HANDLE_VALUE)
	{
		...
		if(hMappedFile)
		{
			if (GetLastError() != ERROR_ALREADY_EXISTS)
			{
				...
				if (addrMappedDll)
				{
                    static Fun_CheckSumMappedFile fun_CheckSumMappedFile = NULL;
                    if (NULL == fun_CheckSumMappedFile)
                    {
                        ...
                        fun_CheckSumMappedFile = (Fun_CheckSumMappedFile)GetProcAddress(hModule, "CheckSumMappedFile");
                        if (NULL == fun_CheckSumMappedFile)
                        {
                            return false;
                        }
                    }
					pNTHeader32 = (PIMAGE_NT_HEADERS32)fun_CheckSumMappedFile(addrMappedDll, fileSize, &headerSum, &checkSum);
